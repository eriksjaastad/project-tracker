...
class DatabaseManager:
    ...
    def log_event(
        ...
        """Log a work event for a project."""
        conn = self._get_conn()
        cursor = conn.cursor()
        ...
        cursor.execute("""
            INSERT INTO work_log (project_id, timestamp, event_type, details)
            VALUES (?, ?, ?, ?)
        """, (project_id, timestamp, event_type, details))
        
        conn.commit()
        conn.close()
    
    def get_work_log(self, project_id: Optional[str] = None, limit: int = 100) -> List[Dict[str, Any]]:
