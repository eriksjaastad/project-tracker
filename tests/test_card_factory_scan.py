...
class TestDeadDepsCheck:
    ...
    def test_skips_uv_export_lockfile(self, tmp_path):
        """A uv export lockfile must not be parsed as direct deps —
        otherwise the entire transitive tree gets flagged as dead.
        Regression test for the muffinpanrecipes 50-finding incident."""
        src = tmp_path / "src"
        src.mkdir()
        (src / "main.py").write_text("import os\n")
        (tmp_path / "requirements.txt").write_text(
            "Resolved 78 packages in 924ms\n"
            "# This file was autogenerated by uv via the following command:\n"
            "#    uv export --no-dev --no-hashes --no-editable\n"
            ".\n"
    ...
    def test_prefers_pyproject_over_requirements(self, tmp_path):
        """When pyproject.toml declares dependencies, requirements.txt
        should be ignored (it's almost always a generated lockfile)."""
        src = tmp_path / "src"
        src.mkdir()
        (src / "main.py").write_text("import anthropic\n")
        (tmp_path / "pyproject.toml").write_text(
            '[project]\nname = "x"\nversion = "0"\n'
            'dependencies = ["anthropic>=0.1", "requests>=2"]\n'
        )
        (tmp_path / "requirements.txt").write_text(
            "anthropic==0.84.0\nrequests==2.31.0\ntqdm==4.66.0\n"
        )
        ...
        pkgs = [f["message"] for f in findings]
        assert any("requests" in m for m in pkgs)
        # tqdm only appears in the lockfile-style requirements.txt, which
        # we now ignore when pyproject has declared deps.
        assert not any("tqdm" in m for m in pkgs)

    def test_detects_indented_imports(self, tmp_path):
        """Imports inside functions (indented) must be detected —
        previously the regex required start-of-line with re.MULTILINE."""
        src = tmp_path / "src"
        src.mkdir()
    ...
    def test_google_genai_import_mapping(self, tmp_path):
        """google-genai is imported as `from google import genai`,
        not as `google_genai`."""
        src = tmp_path / "src"
        src.mkdir()
        (src / "main.py").write_text("from google import genai\n")
        (tmp_path / "pyproject.toml").write_text(
            '[project]\nname = "x"\nversion = "0"\n'
            'dependencies = ["google-genai>=0.1"]\n'
        )
        findings = scan.check_dead_deps(tmp_path)
        assert findings == []
